...
const changePrice = async (e, price) => {
  e.preventDefault();
  await axios.post("http://localhost:5000/post", { value: price });
  ...
  useEffect(() => {
    setTimeout(async () => {
      const fetchData = async () => {
        const res1 = await axios.get(`http://localhost:5000/get/btc`);
        setCrypto(res1.data);
        const res2 = await axios.get(`http://localhost:5000/mode`);
        setMode(res2.data);
        const res5 = await axios.get(`http://localhost:5000/orders`);
